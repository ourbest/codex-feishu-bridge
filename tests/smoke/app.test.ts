...
test('uses openclaw lark send when openclaw config is provided', async () => {
  ...
  const openclawCalls: Array<Record<string, unknown>> = [];
  const app = createBridgeApp({
    config: loadConfig({}),
    larkTransport: transport,
    openclawConfig: { enabled: true },
    openclawLark: {
      async sendMessageFeishu(params) {
        openclawCalls.push(params);
